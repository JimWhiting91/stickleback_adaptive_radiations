#########################################################################################################
# 05 - Harvester. This script imports parallel associated windows and harvests information on genes contained and their GO if available
#########################################################################################################
...
# Define the grouping
grouping_vector<-as.vector(c("50k","75k","100k","200k"))
...
#window_function<-lapply(1:1,function(x){
x <- 1
  ...
  parallel_windows<-lapply(1:length(variables_vector),function(y){
    ...
    # Write to an output
    dir.create(paste0("outputs/",grouping_vector[x],"/biomart/"))
    write.table(parallel_biomart,
                paste0("outputs/",grouping_vector[x],"/biomart/",grouping_vector[x],"_",variables_vector[y],"_parallel_window_genes_biomart_out.txt"),
                row.names = F,sep="\t",quote = F)
    
  })
  
  # Also merge themn all together
  all_windows <- data.frame(rbindlist(parallel_windows))
  
  # Write this
  write.table(all_windows,
              paste0("outputs/",grouping_vector[x],"/biomart/",grouping_vector[x],"_ALL_VARIABLES_parallel_window_genes_biomart_out.txt"),
              row.names = F,sep="\t",quote = F)
#})
