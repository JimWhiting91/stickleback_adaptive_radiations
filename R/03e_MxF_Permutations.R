...
##### First, what is the overlap of MxF outliers ######
  ...
  write.csv(MxF_outlier_out,
              "tables/TableSX_MxF_outliers_50kb.csv",
              row.names = F)
  ...
  ####### Plot a new Figure 2 which includes the Marine Bar ##########
    ...
    # Conf levels
    conf<-unlist(list(c(out_dd$UL),rep(NA,20)))
    # Labels
    asterisk_labs<-c(rep(NA,20),plot_dd[plot_dd$count_type != "Expected","FDR_asterisk"])
