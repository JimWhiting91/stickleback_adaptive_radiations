...
##### Read back in data and calculate the Expected for "ocean level" ie. Alaska and BC OR Scotland and Iceland #####
# Set up PDF for output graphs
pdf("figs/03c_Parallelism_scores_ocean_level.pdf",width=12,height=6)
windows_out<-data.frame(rbindlist(lapply(1:length(grouping_vector),function(x){
  ...
  # Export
  write.table(variables_out,
              paste0("outputs/03_Perm_results_ocean_level_",grouping_vector[x],".txt"),
  ...
  p1<-ggplot(plot_dd,aes(x=var_F,y=Count,fill=count_type))+
    ...
    scale_fill_manual(values = c("grey70", "red4","forestgreen","blue4","gold2")) +
    labs(fill="")+
    ylim(0,(max(plot_dd$Count)+0.2*max(plot_dd$Count)))+
    ylab(paste0("Count of Associated ",grouping_vector[x]," Windows Shared in >1 Radiation"))+
    geom_text(label = asterisk_labs,nudge_y = 1)
...
#### Read back in data and calculate the Expected for " beyond ocean level" ie. NOT Alaska and BC OR Scotland and Iceland #####
#
# Set up PDF for output graphs
pdf("figs/03d_Parallelism_scores_beyondocean_level.pdf",width=12,height=5)
windows_out<-data.frame(rbindlist(lapply(1:length(grouping_vector),function(x){
